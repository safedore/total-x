...
class AddUserBottomSheet extends StatefulWidget {
  const AddUserBottomSheet({super.key, required this.context});
    ...
    return ScrollConfiguration(
      ...
      child: BlocListener<UserBloc, UserState>(
        listenWhen: (previous, current) =>
            previous.createUserStatus != current.createUserStatus,
        listener: (context, state) {
          if (state.createUserStatus is StatusInitial) {
          } else if (state.createUserStatus is StatusLoading) {
            CustomLoading(context: context).show();
          } else if (state.createUserStatus is StatusSuccess) {
            CustomLoading.dismiss(context);
            context.pop();
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text('User updated')));
            context.read<UserBloc>().add(GetUserResultEvent());
          } else {
            CustomLoading.dismiss(context);
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text('User updated failed')));
          }
        },
        child: Container(
              ...
              Align(
                child: GestureDetector(
                  ...
                  child: Column(
                    children: [
                      _image?.path != null
                          ? Image.file(_image!, height: 85.h)
                          : Image.asset(AppImages.pickImage),
                    ...
                    child: CustomButton(
                      ...
                      onPressed: () {
                        ...
                        context.read<UserBloc>().add(
                          CreateUserEvent(userModel: userModel),
                        );
                      },
                      textStyle: AppTypography.montserratMedium.copyWith(
